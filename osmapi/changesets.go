...
func (ChSet *ChangeSetSt) MultiUpload(no_sends ...bool) (map[string] string, error) {
	...
	if no_send {
		...
		return ids, errors.New("Using no_send param")
	}
	...
	s := []string{"relation", "way", "node"}
	for _, v := range s {
		for _, v := range xml_slice(data, "/diffResult/"+v, []string{"old_id", "new_id"}) {
			ids[v["old_id"]] = v["new_id"]
		}
	}
	return ids, nil
}